...
async function promptLicenseKey(keyFromFlag) {
  if (keyFromFlag) {
    ...
    console.log(chalk.gray(`🔑 License key diterima via flag --key`));
    return keyFromFlag;
  }
 ...
 * 
 * ┌─ CATATAN BACKEND ────────────────────────────────────────────┐
 * │ Endpoint ini perlu dibuat di chatbot-plugin-api:             │
 * │                                                              │
...
async function verifyLicenseKey(licenseKey) {
  console.log(chalk.yellow('⏳ Memverifikasi license key ke server...'));
  ...
  try {
    ...
    if (error.response) {
      ...
      if (status === 404) {
        ...
        console.log(
          chalk.yellow(
            '\n⚠  Endpoint /license/verify belum tersedia di backend.'
          ) +
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
      console.log(
        chalk.yellow('\n⚠  Tidak dapat terhubung ke server API FutureCloud.') +
