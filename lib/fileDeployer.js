 ...
 * │ URL ini akan diinjeksi ke dalam file widget chatbot.         │
 * └──────────────────────────────────────────────────────────────┘
 */
const API_URL = 'https://api-chatbot.futurecloud.id/api/v1';

/**
 * Mendeploy file widget chatbot ke proyek user berdasarkan framework.
 * 
...
function deployFile(frameworkMeta, licenseKey) {
  ...
  if (!fs.existsSync(templatePath)) {
    ...
    console.log(
      ...
      chalk.gray(` Menggunakan template universal (chatbot-ui.js)...`)
    );
  ...
  //    Konvensi: FutureCloudChatbot + ekstensi framework
  const outputFilename = `FutureCloudChatbot${frameworkMeta.extension}`;
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
            ...
            <ul className="flex flex-wrap -mb-px text-sm font-medium text-center">
              <li className="mr-2">
                <button 
                  ...
                  className={\`inline-block p-4 border-b-2 rounded-t-lg \${activeTab === 'chatbot' ? 'text-teal-600 border-teal-600' : 'text-gray-500 border-transparent hover:text-gray-600 hover:border-gray-300'}\`}
                >
                  Setup Chatbot
                </button>
              </li>
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            {activeTab === 'chatbot' && (
              <iframe src={\`https://api-chatbot.futurecloud.id/embed/chatbot?license=\${licenseKey}\`} width="100%" height="800px" style={{border: 'none'}}></iframe>
            )}
            {activeTab === 'livechat' && (
              <iframe src={\`https://api-chatbot.futurecloud.id/embed/livechat?license=\${licenseKey}\`} width="100%" height="800px" style={{border: 'none'}}></iframe>
            )}
          </div>
        </div>
      </div>
          ...
          <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
            <li class="mr-2">
              <button 
                ...
                :class="activeTab === 'chatbot' ? 'text-teal-600 border-teal-600' : 'text-gray-500 border-transparent hover:text-gray-600 hover:border-gray-300'"
...
<body class="bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8" x-data="{ activeTab: 'chatbot' }">
        <div class="px-4 py-6 sm:px-0">
            <h1 class="text-3xl font-bold text-gray-900 mb-6">Panel Admin Customer Support</h1>
            ...
            <div class="mb-4 border-b border-gray-200">
                <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
                    <li class="mr-2">
                        <button @click="activeTab = 'chatbot'" :class="activeTab === 'chatbot' ? 'text-teal-600 border-teal-600' : 'text-gray-500 border-transparent hover:text-gray-600 hover:border-gray-300'" class="inline-block p-4 border-b-2 rounded-t-lg">Setup Chatbot</button>
                    </li>
                    <li class="mr-2">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div x-show="activeTab === 'chatbot'">
                    <iframe src="https://api-chatbot.futurecloud.id/embed/chatbot?license={{ $licenseKey ?? '${licenseKey}' }}" width="100%" height="800px" style="border:none;"></iframe>
                </div>
                <div x-show="activeTab === 'livechat'" style="display: none;">
                    <iframe src="https://api-chatbot.futurecloud.id/embed/livechat?license={{ $licenseKey ?? '${licenseKey}' }}" width="100%" height="800px" style="border:none;"></iframe>
                </div>
            </div>
        </div>
    </div>
 ...
 * Ini mempertahankan behavior existing (chatbot-ui.js → futurecloud-chatbot.js).
 * 
...
function deployFallback(frameworkMeta, licenseKey) {
  ...
  const templatePath = path.join(__dirname, '..', 'templates', 'chatbot-ui.js');
  const targetPath = path.join(process.cwd(), 'futurecloud-chatbot.js');
