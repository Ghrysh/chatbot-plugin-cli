...
function deployFallback(frameworkMeta, licenseKey) {
  // ── Logika existing dipertahankan namun menggunakan html ──────────────────────────────
  const templatePath = path.join(__dirname, '..', 'templates', 'chatbot-ui.html');
  const targetPath = path.join(process.cwd(), 'FutureCloudChatbot.html');
  ...
  templateContent = templateContent.replace(/__LICENSE_KEY__/g, licenseKey);
  templateContent = templateContent.replace(/__API_URL__/g, API_URL);
