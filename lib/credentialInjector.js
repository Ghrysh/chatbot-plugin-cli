...
const API_URL_VALUE = 'http://localhost:8001/api/v1'; // ← Ganti dengan URL production
...
function injectToEnv(licenseKey) {
  ...
  if (keyRegex.test(envContent)) {
    ...
    const newEntry =
      `\n# === FutureCloud Chatbot Plugin ===\n` +
      `${ENV_KEY_NAME}=${licenseKey}\n` +
      `${API_URL_KEY_NAME}=${API_URL_VALUE}\n`;

    envContent += newEntry;
    console.log(chalk.green(`   ✔  ${ENV_KEY_NAME} ditambahkan ke .env`));
  }
  ...
  if (!envExists) {
    console.log(chalk.gray(`   📄 File .env baru dibuat di: ${envPath}`));
  }
  ...
  if (fs.existsSync(envExamplePath)) {
    ...
    if (!exampleKeyRegex.test(exampleContent)) {
      ...
      console.log(chalk.gray(`   📄 .env.example juga diperbarui`));
    }
  }
 ...
 * Script tag berisi referensi ke file FutureCloudChatbot.html yang sudah digenerate.
 * 
...
function injectScriptTagToHtml(targetWidgetPath, htmlFilePath = null) {
  ...
  if (!resolvedHtmlPath || !fs.existsSync(resolvedHtmlPath)) {
    ...
    console.log(chalk.yellow(`\n⚠  File HTML tidak ditemukan untuk injeksi otomatis.`));
  ...
  // Cek apakah sudah ada script chatbot agar tidak duplikasi
  if (htmlContent.includes('FutureCloud Chatbot Plugin')) {
    console.log(chalk.yellow(`   ⚠  Script chatbot sudah ada di ${path.basename(resolvedHtmlPath)}, dilewati.`));
    return { injected: false };
  }
  ...
  if (htmlContent.includes('</body>')) {
    ...
    console.log(chalk.green(`   ✔  Script tag berhasil disisipkan ke: ${resolvedHtmlPath}`));
  ...
  fs.writeFileSync(resolvedHtmlPath, htmlContent, 'utf8');
  console.log(chalk.green(`   ✔  Script tag ditambahkan ke akhir: ${resolvedHtmlPath}`));
  return { injected: true, htmlPath: resolvedHtmlPath };
}
