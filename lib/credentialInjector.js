...
const API_URL_VALUE = 'https://api-chatbot.futurecloud.id/api/v1'; // ← Ganti dengan URL production
...
function injectScriptTagToHtml(targetWidgetPath, htmlFilePath = null) {
  ...
  if (htmlContent.includes('FutureCloud Chatbot Plugin')) {
    console.log(chalk.yellow(`   [WARNING]  Script chatbot sudah ada di ${path.basename(resolvedHtmlPath)}, dilewati.`));
    return { injected: false };
  }
